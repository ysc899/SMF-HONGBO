					...
					<form id="editForm">
						...
						<table>
							...
							<tbody>
								...
								<tr>
									<th>이메일</th>
									<td>
										<input type="text" class="form-control text_title" name="email" placeholder="담당자 이메일을 입력해주세요." id="email" value='<c:out value="${result.email }"/>'>
									</td>
								</tr>
							...
							<tbody>
								<tr>
									...
									<td>
										...
										<input type="hidden" name="<c:out value="${item.slaveCode }"/>_seq" id="<c:out value="${item.slaveCode }"/>_seq" value="0">
									</td>
								</tr>
								<tr class="">
									<th>사업소 명</th>
						...
						<table>
							...
							<tbody>
								...
								<tr>
									<th><label for="loc_<c:out value='${item.slaveCode }'/>"><c:out value='${item.slaveCodeName }'/></label><input class="_locCheckBox" type="checkbox" value="<c:out value='${item.slaveCode }'/>" data-name="<c:out value='${item.slaveCodeName }'/>" data-seq="0" id="loc_<c:out value='${item.slaveCode }'/>"></th>
									<td>
	...
	<script type="text/javascript">
		...
		<c:if test="${item.codeLangType == 'ko' }">
			staffsInfos.push({
				...
				department: '<c:out value="${item.department }"/>',
				company: '<c:out value="${item.company }"/>',
				address: '<c:out value="${item.address }"/>',
				mobilePhone: '<c:out value="${item.mobilePhone }"/>',
				fax: '<c:out value="${item.fax }"/>',
			});
		</c:if>
		</c:forEach>
		...
		<c:forEach var="item" items="${result.locInfos }" >
			locInfos.push({
				seq: <c:out value="${item.seq }"/>,
				codeNetworkType: '<c:out value="${item.codeNetworkType }"/>',
				codeNetworkName: '<c:out value="${item.codeNetworkName }"/>',
		...
		function printStaffsInfo(){
			for(var idx in staffsInfos){
				...
				$(domPrefix+"department").val(item.department);
				$(domPrefix+"company").val(item.company);
				$(domPrefix+"address").val(item.address);
				$(domPrefix+"mobilePhone").val(item.mobilePhone);
				$(domPrefix+"fax").val(item.fax);
			}
		}
		
		function printLocInfo(){
			for(var idx in locInfos){
				let item = locInfos[idx];
				console.log(item);
				let domSubfix = "_"+item.codeNetworkType;
				$("#loc"+domSubfix).prop("checked", "checked");
				$("#loc"+domSubfix).data('seq', item.seq);
				$("#locCount"+domSubfix).attr( 'disabled', false );
				$("#locCount"+domSubfix + " option:last").remove();
				$("#locCount"+domSubfix).val(item.printOrder).attr("selected", "selected");
			}
		}
		...
		$(document).ready(function(){
			...
			<c:if test="${result.directorFlag == 'Y'}">
				$("#directorFlag").prop("checked", "checked");
				$("._directorThum").show();
			</c:if>
			...
			$("._locCheckBox").change(function(){
				...
				if(isCheck){
					if(seq > 0){
						var index = deleteLoc.indexOf(seq);
						if (index !== -1) deleteLoc.splice(index, 1);
					}
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					fd.append('locDeleteJSON', JSON.stringify(deleteLoc));
					
					
					axios.post("<c:url value='/admin/status/network/editNetwork.edit' />", fd,{headers : {
