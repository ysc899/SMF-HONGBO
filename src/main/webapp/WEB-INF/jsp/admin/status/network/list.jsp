					...
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘"><c:out value="${nowMenuName }"/> - 리스트</h1>
						<div class="select_box">
							...
							<form id="searchForm" method="get">
								...
								<select class="kind" name="searchOption" id="optSearch">
									...
									<option value="company">사업소명</option>
								</select>
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;담당자 추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
...
// 				location.href="<c:url value='/admin/status/network/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/status/network/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/status/network/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/status/network/add.write' />").submit();
