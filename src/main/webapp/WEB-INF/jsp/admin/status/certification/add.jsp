...
<!-- 										<input type="text" class="form-control text_title" name="category" placeholder="등록될 카테고리를 입력해주세요." id="category"> -->
									</td>
								</tr>
								<tr>
									<th>분류</th>
								...
								<tr>
									<th>발급기관</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="issuingAgency" placeholder="등록될 발급기간 입력해주세요." id="issuingAgency">
									</td>
								</tr>
								<tr>
									<th>인증내역</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="content" placeholder="등록될 내용을 입력해주세요." id="content">
									</td>
								</tr>
								<tr>
									<th>인증서 이미지</th>
		...
		$(document).ready(function(){
			...
			$("#addForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/status/certification/addCertification.write' />", fd,{headers : {
						'Content-Type' : 'multipart/form-data'
					}})
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('등록 되었습니다.');
//  					    	location.href="<c:url value='/admin/status/certification/" + res.data.result + ".view' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/status/certification/" + res.data.result + ".view' />").submit();
