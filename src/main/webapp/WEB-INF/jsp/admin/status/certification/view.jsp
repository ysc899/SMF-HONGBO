...
<html>
				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>센터</th>
								<td colspan="3"><c:out value="${result.codeCenterName }"/></td>
							</tr>
							<tr>
								<th>분류</th>
								<td colspan="3"><c:out value="${result.codeCertName }"/></td>
							</tr>
							<tr>
								<th>국내외</th>
								<td colspan="3"><c:out value="${result.codeCertLocName }"/></td>
							</tr>
							<tr>
								<th>타이틀</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.title }"/></td>
							</tr>
							<tr>
								<th>발급기관</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.issuingAgency }"/></td>
							</tr>
							<tr>
								<th>인증내역</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 인증현황 입니다.');
				location.href="<c:url value='/admin/status/certification/index.do' />";
			}
			
			//목록 가기 클릭 이벤트
			$("._list").click(function(){
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/status/certification/index.do' />").submit();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('인증현황 정보를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/status/certification/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/status/certification/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/status/certification/"+ seq +".edit' />").submit();
				});
