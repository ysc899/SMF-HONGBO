					...
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘"><c:out value="${nowMenuName }"/> - 리스트</h1>
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><c:out value="${item.codeCertName }"/></td>
										<td><c:out value="${item.codeCertLocName }"/></td>
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;인증현황 추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
...
// 				location.href="<c:url value='/admin/status/certification/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/status/certification/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/status/certification/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/status/certification/add.write' />").submit();
