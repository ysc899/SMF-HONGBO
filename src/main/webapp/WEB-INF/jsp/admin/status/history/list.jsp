...
<!-- 								</select> -->
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;연혁 추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
...
// 				location.href="<c:url value='/admin/status/history/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/status/history/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/status/history/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/status/history/add.write' />").submit();
