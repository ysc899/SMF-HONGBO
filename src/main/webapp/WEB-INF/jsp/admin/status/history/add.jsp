						...
						<table>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<div class="input-group date">
										    <input type="text" class="form-control _datepicker" name="selectDate" autocomplete="off" id="selectDate">
								...
								<tr>
									<th>내용</th>
									<td>
										<input type="text" class="form-control text_title" name="content" placeholder="등록될 연혁 내용을 입력해주세요." id="title">
									</td>
								</tr>
								<tr>
									<th>출력 순서</th>
		...
		$(document).ready(function(){
			...
			$("#addForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/status/history/addHistory.write' />", param)
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('등록 되었습니다.');
//  					    	location.href="<c:url value='/admin/status/history/" + res.data.result + ".view' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/status/history/" + res.data.result + ".view' />").submit();
