...
<html>
				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<c:if test="${result.endYear != '' }">
							<tr >
								<th>종료 년도</th>
								<td colspan="3"><c:out value="${result.endYear }"/></td>
							</tr>
							</c:if>
							<tr>
								<th>내용</th>
							...
							<tr>
								<th>출력 순서</th>
								<td colspan="3"><c:out value="${result.printOrder }"/></td>
							</tr>
							<tr>
								<th>최종 수정자</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 연혁정보 입니다.');
				location.href="<c:url value='/admin/status/history/index.do' />";
			}
			
			//목록 가기 클릭 이벤트
			$("._list").click(function(){
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/status/history/index.do' />").submit();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('연혁 정보를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/status/history/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/status/history/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/status/history/"+ seq +".edit' />").submit();
