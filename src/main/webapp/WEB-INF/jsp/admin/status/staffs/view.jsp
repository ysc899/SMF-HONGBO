...
<html>
		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘"><c:out value="${nowMenuName }"/> - 상세 조회 - 공통 정보</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="*">
						...
						<tbody>
							<tr>
								<th>센터코드</th>
								<td><c:out value="${result.codeCenterName }"/></td>
								<th>부서코드</th>
								<td><c:out value="${result.codePartName }"/></td>
							</tr>
							<tr>
								<th>대표원장 여부</th>
								<td ><c:out value="${result.masterFlag }"/></td>
								<th>출력 순서</th>
							...
							<tr >
								<th>섬네일 이미지</th>
								<td colspan="3"><img src='<c:out escapeXml="false" value="${result.thumImageContent }"/>' /></td>
							</tr>
							</c:if>
					...
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘"> 의료진 <c:out value="${item.codeLangName }"/> 정보</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="*">
					...
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘"> 의료진 약력 정보</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="100px;">
						...
						<tbody>
							...
							<tr>
								<td><c:out value="${item.codeLangName }"/></td>
								<td><c:out value="${item.nowFlag }"/></td>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 의료진정보 입니다.');
				location.href="<c:url value='/admin/status/staffs/index.do' />";
			}
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('의료진 정보를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/status/staffs/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/status/staffs/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/status/staffs/"+ seq +".edit' />").submit();
