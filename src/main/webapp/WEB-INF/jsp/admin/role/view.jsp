		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">권한 상세 조회</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="*">
						...
						<tbody>
							<tr>
								<th>권한명</th>
								<td colspan="3"><c:out value="${result.role.roleName }"/></td>
							</tr>
							<tr>
								<th>설명</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.role.roleDesc }"/></td>
							</tr>
							<tr>
								<th>최종 수정자</th>
								<td><c:out value="${result.role.editUserName }"/>&nbsp;(&nbsp;<c:out value="${result.role.editUserId }"/>&nbsp;)</td>
								<th>최종 수정 일시</th>
								<td><fmt:formatDate pattern = "yyyy-MM-dd hh:mm:ss" value = "${result.role.editDate}" /></td>
							</tr>
						</tbody>
					</table>
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					...
					if(confirm(roleName + ' 사용자를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/role/"+ roleName +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/role/"+ roleName +".edit' />";
			    	$("#hiddenForm").attr('action', "<c:url value='/admin/role/"+ roleName +".edit' />").submit();
