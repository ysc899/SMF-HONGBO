...
<html>
		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">권한 관리</h1>
						<div class="select_box">
							...
							<form id="searchForm" method="get">
								...
								<select class="kind" name="searchOption" id="optSearch">
									...
									<option value="desc">권한 설명</option>
								</select>
						...
						<table>
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-id="<c:out value="${item.roleName }"/>">
										<td><c:out value="${item.row }"/></td>
										<td><a href="javascript:void(0);" style="color:#000;"><c:out value="${item.roleName }"/></a></td>
						...
						<div class="page_box">
							...
							<button id="btnAddRole" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-user-plus"></i>&nbsp;권한 추가</button>
						</div>
				</div>
				<!--/notice_area End-->
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("._row").click(function(){
				let id = $(this).data('id');
// 				location.href="<c:url value='/admin/role/" + id + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/role/" + id + ".view' />").submit();
			});
...
// 					location.href="<c:url value='/admin/role/addRole.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/role/addRole.write' />").submit();
