		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">One Click 건강정보 관리</h1>
						<div class="select_box">
							...
							<form id="searchForm" method="get">
								...
								<select class="kind" name="searchOption" id="optSearch">
									...
									<option value="subtitle">보조타이틀</option>
								</select>
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><c:out escapeXml="false" value="${item.subTitle }"/></td>
										<td><c:out value="${item.printOrder }"/></td>
										<td><c:out value="${item.printFlag }"/></td>
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddAdmin" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;건강정보 추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
...
// 				location.href="<c:url value='/admin/health/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/health/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/health/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/health/add.write' />").submit();
