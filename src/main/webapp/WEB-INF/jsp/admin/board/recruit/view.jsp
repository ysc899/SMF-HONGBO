...
<html>
				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>채용 종류</th>
								<td><c:out value="${result.codeRecruitName }"/></td>
								<th>채용 상태</th>
								<td><c:out value="${result.codeRecruitStateName }"/></td>
							</tr>
							<tr>
								<th>표시 여부</th>
								<td><c:out value="${result.viewFlag }"/></td>
								<th>조회 수</th>
								<td><c:out value="${result.hitCnt }"/></td>
							</tr>
							<tr>
								<th>시작일</th>
								<td><fmt:formatDate pattern = "yyyy-MM-dd" value = "${result.startDate}" /></td>
								<th>종료일</th>
								<td><fmt:formatDate pattern = "yyyy-MM-dd" value = "${result.endDate}" /></td>
							</tr>
							<tr>
								<th>타이틀</th>
								<td colspan="3"><c:out value="${result.title }"/></td>
							</tr>
							<c:if test="${result.inputTypeFlag eq 'N' }">
							<tr>
								<th>내용</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.content }"/></td>
							</tr>
							</c:if>
							<c:if test="${result.inputTypeFlag eq 'Y' }">
							<tr>
								<th>내용 타이틀</th>
								<td colspan="3">(재)씨젠의료재단 <c:out value="${result.contentTitle }"/> 전문인력채용</td>
							</tr>
							<tr>
								<th>채용 부문</th>
								<td colspan="3">(재)씨젠의료재단 <c:out value="${result.contentComment }"/> 도전적이고 진취적인 우수인재를 모십니다.</td>
							</tr>
							</c:if>
							<tr>
								<th>최종 수정자</th>
					...
					<c:if test="${result.inputTypeFlag eq 'Y' }">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">업무내용</h1>
					<table>
						<colgroup>
							<col width="50px;"/>
							<col width="10%;"/>
						...
						<tbody>
							<c:forEach var="item" items="${result.tasks }" varStatus="status">
								<tr>
									...
									<td><c:out value="${item.taskSection}"/></td>
									<td><c:out value="${item.taskLoc}"/></td>
									<td><c:out value="${item.taskAssigned}"/></td>
									<td><c:out escapeXml="false" value="${item.taskTime}"/></td>
									<td><c:out escapeXml="false" value="${item.taskRequire}"/></td>
									<td><c:out escapeXml="false" value="${item.taskOption}"/></td>
									<td><c:out value="${item.taskApes}"/></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 채용정보 입니다.');
				location.href="<c:url value='/admin/board/recruit/index.do' />";
			}
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm(title + ' 채용 정보를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/board/recruit/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/board/recruit/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/board/recruit/"+ seq +".edit' />").submit();
