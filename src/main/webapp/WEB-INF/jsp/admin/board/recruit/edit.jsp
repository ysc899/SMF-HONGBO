		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<input type="checkbox" id="inputTypeFlag" disabled="disabled">
									</td>
									<th>표출 여부</th>
									<td>
								...
								<c:if test="${result.inputTypeFlag == 'N' }">
								<tr>
									<th>내용</th>
									<td class="write_note" colspan="3"><div id="contentSummernote"></div></td>
								</tr>
								</c:if>
								<c:if test="${result.inputTypeFlag == 'Y' }">
								<tr>
									<th>내용 타이틀</th>
									<td class="write_note" colspan="3">(재)씨젠의료재단 <input type="text" class="form-control text_title" name="contentTitle" placeholder="내용을 입력해주세요." id="contentTitle" value="<c:out value='${result.contentTitle }'/>"> 전문인력채용</td>
								</tr>
								<tr class="_inputYType">
									<th>채용 부문</th>
									<td class="write_note" colspan="3">(재)씨젠의료재단 <input type="text" class="form-control text_title" name="contentComment" placeholder="내용을 입력해주세요." id="contentComment" value="<c:out value='${result.contentComment }'/>"> 도전적이고 진취적인 우수인재를 모십니다.</td>
								</tr>
								</c:if>
							</tbody>
						</table>
					...
					<c:if test="${result.inputTypeFlag == 'Y' }">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">업무내용 등록</h1>
					<table>
						<colgroup>
							<col width="50px;"/>
							<col width="10%;"/>
			...
			<div class="modal-dialog" role="document">
			  <div class="modal-content">
			    ...
			    <div class="modal-body">
			    	<form id="addTaskForm">
			    		<input type="hidden" name="seq" id="taskSeq">
			    		<input type="hidden" name="row" id="taskRow">
				    	<table class="table">
				    		<tbody>
				    			<tr>
				    				<th>채용 부문</th>
	...
	<script type="text/javascript">
		...
		function printTaskList(){
			...
			for(idx in taskInfos){
				...
				sourceHtml += '	<td class="_taskRow" data-seq="'+ item.seq +'" data-row="'+ item.row +'">'+item.taskSection+'</td>';
				...
				if(item.seq > 0){
					sourceHtml += '<td><button class="btn btn-warning _btnDeleteTask" data-seq="'+item.seq +'" data-row="'+ item.row +'">삭제</button></td>';
				}else{
					sourceHtml += '<td><button class="btn btn-warning _btnRemoveTask" data-row="'+ item.row +'">삭제</button></td>';
				}
				sourceHtml += '</tr>';
			}
		...
		//채용 업무 디비 삭제 이벤트
		$(document).on('click', '._btnDeleteTask',function(){
			var row = $(this).data('row');
			...
			printTaskList();
			deleteTesks.push(seq);
		});
		
		//채용 업무 팝업 표시 이벤트
		$(document).on('click', '._taskRow',function(){
			var row = $(this).data('row');
			var seq = $(this).data('seq');
			var idx = Number(row) -1;
			var item = taskInfos[idx];
...
// 			console.log(item);
			//입력값 초기화
			$("#taskSection").val(item.taskSection);
			$("#taskLoc").val(item.taskLoc);
			$("#taskAssigned").val(item.taskAssigned);
			$("#taskRequire").val(CommonUtil.toEnter(item.taskRequire));
			$("#taskOption").val(CommonUtil.toEnter(item.taskOption));
			$("#taskApes").val(item.taskApes);
			$("#taskTime").val(CommonUtil.toEnter(item.taskTime));
		...
		$(document).ready(function(){
			...
			//채용 종류 초기화
			$("#codeRecruitTP").val('<c:out value="${result.codeRecruitType}"/>').attr("selected", "selected");
			//채용 상태 초기화
			$("#codeRecruitST").val('<c:out value="${result.codeRecruitStateType}"/>').attr("selected", "selected");
			
			//표출 여뷰 처리
			if('<c:out value="${result.viewFlag}"/>' == 'Y' ){
				$("#viewFlag").prop("checked", "checked");
			}
			//입력 방식 여부 처리
			if('<c:out value="${result.inputTypeFlag}"/>' == 'Y' ){
				$("#inputTypeFlag").prop("checked", "checked");
			}
			...
			$("#contentSummernote").summernote("code", '<c:out escapeXml="false" value="${result.content }"/>');
			</c:if>
			
			<c:if test="${result.inputTypeFlag == 'Y' }">
				//업무내용 등록
				<c:forEach var="item" items="${result.tasks }" varStatus="status">
				taskInfos.push({
					seq: <c:out value="${item.seq}"/>,
					row: taskInfos.length+1,
					taskSection: '<c:out value="${item.taskSection}"/>',
					taskLoc: '<c:out value="${item.taskLoc}"/>',
					taskAssigned: '<c:out value="${item.taskAssigned}"/>',
					taskTime: '<c:out escapeXml="false" value="${item.taskTime}"/>',
					taskRequire: '<c:out escapeXml="false" value="${item.taskRequire}"/>',
					taskOption: '<c:out escapeXml="false" value="${item.taskOption}"/>',
					taskApes: '<c:out value="${item.taskApes}"/>'
				});
				</c:forEach>
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					if(isInputType){
						...
						param.deleteTaskJSON = JSON.stringify(deleteTesks);
					...
					axios.post("<c:url value='/admin/board/recruit/editRecruit.edit' />", param)
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/board/recruit/${result.seq}.view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/board/recruit/${result.seq}.view' />").submit();
