...
<html>
		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">급여 고시  상세 조회</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="*">
						...
						<tbody>
							<c:forEach var="item" items="${result.fileList }" varStatus="status">
								<tr>
									...
									<td><a href="<c:url value='/admin/board/payrollNotice/download/${result.seq }/${item.seq }.do'/>"><c:out value="${item.fileName}"/></a></td>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 급여 고시  입니다.');
				location.href="<c:url value='/admin/board/payrollNotice/index.do' />";
			}
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm(title + ' 급여 고시 를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/board/payrollNotice/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/board/notice/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/board/payrollNotice/"+ seq +".edit' />").submit();
