...
<html>
			...
			<div id="container">
				...
				<div class="notice_area">
					<form id="editForm">
						...
						<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">급여 고시 수정</h1>
						<table>
							<colgroup>
								<col width="100px;"/>
								<col width="400px;"/>
							...
							<tbody>
								...
								<tr>
									<th>고시 번호</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="payrollNoticeNumber" placeholder="등록될 급여 고시번호를 입력해주세요." id="payrollNoticeNumber" value='<c:out value="${result.payrollNoticeNumber }"/>'>
									</td>
								</tr>
								<tr>
									<th>타이틀</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			//공지 여뷰 처리
			if('<c:out value="${result.payrollNoticeFlag}"/>' == 'Y' ){
				$("#payrollNoticeFlag").prop("checked", "checked");
			}
			...
			$("#editForm").validate({
				rules:{
					title: {required:true},
					payrollNoticeNumber: {required:true, minlength:2, maxlength:100}
				},
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/board/payrollNotice/editPayrollNotice.edit' />", fd,{headers : {
...
//  					    	location.href="<c:url value='/admin/board/notice/" + seq + ".view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/board/payrollNotice/" + seq + ".view' />").submit();
