		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">급여 고시 관리</h1>
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><c:out value="${item.payrollNoticeNumber }"/></td>
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;고시 추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
...
// 				location.href="<c:url value='/admin/board/notice/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/board/payrollNotice/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/board/notice/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/board/payrollNotice/add.write' />").submit();
