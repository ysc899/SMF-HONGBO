						...
						<table>
							...
							<tbody>
								...
								<tr>
									<th>게시판 명</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="name" placeholder="등록될 게시판명을 입력해주세요." id="name" value='<c:out value="${result.masterName }"/>'>
									</td>
								</tr>
								<tr>
									<th>게시판 설명</th>
	...
	var seq = "<c:out value='${result.seq }'/>";
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				...
				location.href="<c:url value='/admin/board/manage/index.do' />";
			}
			
			//언어코드 초기화
			$("#codeLang").val('<c:out value="${result.codeLangType}"/>').attr("selected", "selected");
			
			var desc = '<c:out escapeXml="false" value="${result.masterDesc }"/>';
			$("#desc").val(CommonUtil.toEnter(desc));
			
			if('<c:out value="${result.optThumFlag }"/>' == 'Y' ){
				$("#optThum").prop('checked', true);
			}
			if('<c:out value="${result.optFileFlag }"/>' == 'Y' ){
				$("#optFile").prop('checked', true);
			}
			if('<c:out value="${result.optNotiFlag }"/>' == 'Y' ){
				$("#optNoti").prop('checked', true);
			}
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/board/manage/editBoardManage.edit' />", param)
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/board/manage/" + seq + ".view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/board/manage/" + seq + ".view' />").submit();
