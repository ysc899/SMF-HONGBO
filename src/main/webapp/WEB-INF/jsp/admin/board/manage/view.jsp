...
<html>
				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>게시판 명</th>
								<td><c:out value="${result.masterName }"/></td>
								<th>글 수</th>
								<td><c:out value="${result.contentCount }"/></td>
							</tr>
							<tr>
								<th>섬네일 사용 여부</th>
								<td><c:out value="${result.optThumFlag }"/></td>
								<th style="font-size:15px;">첨부파일 사용 여부</th>
								<td><c:out value="${result.optFileFlag }"/></td>
							</tr>
							<tr>
								<th>공지 사용 여부</th>
								<td><c:out value="${result.optNotiFlag }"/></td>
							</tr>
							<tr>
								<th>설명</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.masterDesc }"/></td>
							</tr>
							<tr>
								<th>최종 수정자</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			var boardName = '<c:out value="${result.masterName }"/>';
			if(seq == ''){
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm(boardName + ' 게시판을 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/board/manage/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.\n관리자 메뉴는 직접 삭제 하셔야 합니다.');
// 						    	history.back();
								$("#hiddenForm").attr('action', "<c:url value='/admin/board/manage/index.do' />").submit();
...
// 					location.href="<c:url value='/admin/board/manage/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/board/manage/"+ seq +".edit' />").submit();
