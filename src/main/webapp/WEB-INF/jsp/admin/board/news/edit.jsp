							...
							<tbody>
							 <tr>
								...
								<tr>
									<th>시스템 구분</th>
									<td>
										<select class="form-control" id="codeCenter" name="systemType">
											<c:if test='${result.systemType =="ALL"}'>
												<option value="ALL" selected="selected">전체</option>
										    	<option value="TEST_RESULT">검사 결과관리 </option>
										    	<option value="MARKETING">홍보 홈페이지</option>
											</c:if>
											<c:if test='${result.systemType =="TEST_RESULT"}'>
												<option value="ALL">전체</option>
										    	<option value="TEST_RESULT" selected="selected">검사 결과관리 </option>
										    	<option value="MARKETING">홍보 홈페이지</option>
											</c:if>
											<c:if test='${result.systemType =="MARKETING"}'>
												<option value="ALL">전체</option>
										    	<option value="TEST_RESULT">검사 결과관리 </option>
										    	<option value="MARKETING" selected="selected">홍보 홈페이지</option>
											</c:if>
										</select>
									</td>
									<th>팝업 생성</th>
									<td>
										<select class="form-control" id="codeCenter" name="popupUseYn">
										   <c:if test="${result.popupUseYn == 'Y'}">
										    <option value="N">미생성</option>		
										    <option value="Y" selected="selected">생성</option>
										   </c:if>
										   <c:if test="${result.popupUseYn == 'N'}">
											<option value="N" selected="selected">미생성</option>
								...
								<tr>
									...
									<td>
										<div class="input-group date">                                        
										    <input type="text" class="form-control _datepicker" name="popupStartDate" autocomplete="off" id="startDate" value='<fmt:formatDate pattern = "yyyy-MM-dd" value = "${result.popupStartDate}" />'>
									...
									<td>
										<div class="input-group date">
										    <input type="text" class="form-control _datepicker" name="popupEndDate" autocomplete="off" id="endDate"  value='<fmt:formatDate pattern = "yyyy-MM-dd" value = "${result.popupEndDate}" />'>
										    <div class="input-group-addon">
										        <span class="glyphicon glyphicon-th"></span>
										    </div>
										</div>
								...
								<tr>
									<th>좌측좌표</th>
									<td>                                                           
										<input type="text" class="form-control text_title" name="popupLeftPosition" id="popupLeftPosition" value='<c:out value="${result.popupLeftPosition}"/>' style="width:300px;">
									</td>
									<th>상단좌표</th>
									<td><%-- <c:out value="${item.slaveCodeName }/> --%>
										<input type="text" class="form-control text_title" name="popupTopPosition" id="popupTopPosition" value='<c:out value="${result.popupTopPosition}"/>' style="width:300px;">
									</td>
								</tr>
								<tr>
									<th>내용</th>
							...
							<tbody>
								<c:forEach var="item" items="${result.files }" varStatus="status">
									<tr>
										...
										<td colspan="2"><a href="<c:url value='/admin/board/news/${masterSeq}/download/${result.seq }/${item.seq }.do'/>"><c:out value="${item.fileName}"/></a></td>
		...
		$(document).ready(function(){
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/board/news/${masterSeq}/editBoardIntegrated.edit' />", fd,{headers : {
...
//  					    	location.href="<c:url value='/admin/board/integrated/${masterSeq}/${result.seq}.view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/board/news/${masterSeq}/${result.seq}.view' />").submit();
