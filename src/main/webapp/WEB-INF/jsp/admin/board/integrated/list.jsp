					...
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘"><c:out value="${nowMenuName }"/> - 리스트</h1>
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><a href="javascript:void(0);" style="color:#000;"><c:out value="${item.title }"/></a></td>
										
										<c:if test='${option.optNotiFlag == "Y" }'>
											<td><c:out value="${item.noticeFlag }"/></td>
										</c:if>
										<c:if test='${item.langType == "ko" }'>
											<td>한국어</td>
										</c:if>
										<c:if test='${item.langType == "en" }'>
											<td>영어</td>
										</c:if>
										
										<td><c:out escapeXml="false" value="${item.hitCnt }"/></td>
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
...
// 				location.href="<c:url value='/admin/board/integrated/${masterSeq}/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/board/integrated/${masterSeq}/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/board/integrated/${masterSeq}/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/board/integrated/${masterSeq}/add.write' />").submit();
