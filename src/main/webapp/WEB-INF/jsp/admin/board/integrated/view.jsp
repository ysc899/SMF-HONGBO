						...
						<tbody>
							<c:forEach var="item" items="${result.files }" varStatus="status">
								<tr>
									...
									<td><a href="<c:url value='/admin/board/integrated/${masterSeq}/download/${result.seq }/${item.seq }.do'/>"><c:out value="${item.fileName}"/></a></td>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				...
				location.href="<c:url value='/admin/board/integrated/${masterSeq}/index.do' />";
			}
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('내용을 삭제 하시겠습니까?')){
						...
						axios.post("<c:url value='/admin/board/integrated/${masterSeq}/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/board/integrated/${masterSeq}/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/board/integrated/${masterSeq}/"+ seq +".edit' />").submit();
