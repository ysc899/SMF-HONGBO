...
<html>
		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">공문 상세 조회</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="*">
						...
						<tbody>
							<tr>
								...
								<th>언어코드</th>
								<td><c:out value="${result.codeLangName }"/></td>
							</tr>
							<tr>
								<th>공지여부</th>
								<td>
								   <c:if test="${result.noticeFlag == 'Y'}">
								   	공지
								   </c:if>
								   <c:if test="${result.noticeFlag == 'N'}">
									미공지		
								   </c:if>
								</td>
								<th>조회 수</th>
							...
							<tr>
								...
								<th>공문번호</th>
								<td><c:out value="${result.noticeNumber }"/></td>
							</tr>
							<tr>
								<th>타이틀</th>
						...
						<tbody>
							<c:forEach var="item" items="${result.fileList }" varStatus="status">
								<tr>
									...
									<td><a href="<c:url value='/admin/board/notice/download/${result.seq }/${item.seq }.do'/>"><c:out value="${item.fileName}"/></a></td>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 공문 입니다.');
				location.href="<c:url value='/admin/board/notice/index.do' />";
			}
			
			//목록 가기 클릭 이벤트
			$("._list").click(function(){
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/board/notice/index.do' />").submit();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm(title + ' 공문을 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/board/notice/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/board/notice/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/board/notice/"+ seq +".edit' />").submit();
