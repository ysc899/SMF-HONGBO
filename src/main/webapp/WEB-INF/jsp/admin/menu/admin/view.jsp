...
<html>
		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">관리자 메뉴 상세 조회</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="*">
						...
						<tbody>
							...
							<tr>
								<th>출력 순서</th>
								<td colspan="3"><c:out value="${result.menuOrder }"/></td>
							</tr>
							<tr>
								<th>URL</th>
								<td colspan="3"><c:out value="${result.menuUrl }"/><c:if test="${result.parentSeq > 0 }">index.do</c:if> </td>
							</tr>
							<tr>
								<th>최종 수정자</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 관리자 메뉴 입니다.');
				location.href="<c:url value='/admin/menu/admin/index.do' />";
			}
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					...
					if(confirm(menuName + ' 메뉴를 삭제 하시겠습니까?')){
						...
						axios.post("<c:url value='/admin/menu/admin/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/menu/admin/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/menu/admin/"+ seq +".edit' />").submit();
