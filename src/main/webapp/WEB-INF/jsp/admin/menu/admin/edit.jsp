...
<html>
					...
					<form id="editForm">
						...
						<table>
							...
							<tbody>
								...
								<tr>
									<th>URL</th>
									<td colspan="3"><input type="text" class="form-control text_title" name="url" placeholder="메뉴 URL를 입력 하세요" id="url" value="<c:out value='${result.menuUrl}'/><c:if test="${result.parentSeq > 0 }">index.do</c:if>"></td>
								</tr>
							</tbody>
						</table>
					</form>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			//상위 메뉴 선택 처리
			$("#parent").val('<c:out value="${result.parentSeq }"/>').attr("selected", "selected");
			//출력 순서 조정 이벤트
			setMenuPrintOrder('<c:out value="${result.parentSeq }"/>');
			//출력 순서 선택 처리
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/menu/admin/editMenu.edit' />", param)
					  .then(function (res) {
					    if(res.data.result){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/menu/admin/" + seq + ".view' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/menu/admin/" + seq + ".view' />").submit();
