...
<html>
			...
			<div id="container">
				...
				<div class="notice_area">
					<form id="editForm">
						...
						<sec:csrfInput />
						<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">사용자 메뉴 수정</h1>
						<table>
							<colgroup>
								<col width="100px;"/>
								<col width="400px;"/>
							...
							<tbody>
								<tr>
									...
									<th>메뉴명</th>
									<td>
										<input type="text" class="form-control text_title" name="menuName" placeholder="등록될 메뉴명을 입력해주세요." id="menuName" value="<c:out value='${result.menuName}'/>">
									</td>
								</tr>
								<tr>
									<th>메뉴코드</th>
								...
								<tr>
									<th>상위 메뉴 선택</th>
									<td>
										<input type="hidden" name="parentSeq" id="parentSeq" value="<c:out value='${result.parentSeq }'/>">
								...
								<tr>
									<th>URL</th>
									<td colspan="3"><input type="text" class="form-control text_title" name="url" placeholder="메뉴 URL를 입력 하세요" id="url" value="<c:out value='${result.menuUrl}'/>"></td>
								</tr>
							</tbody>
						</table>
					</form>
	...
	<script type="text/javascript">
		...
		//출력 순서 갯수 조절 메뉴 (다른 메뉴 선택 시)
		function setMenuPrintOrder(size){
			
			setMenuPrintSize(size+1)
		}
		//출력 순서 갯수 조절
		function setMenuPrintSize(selectCount){
			$("#menuOrder").empty();
		...
		$(document).ready(function(){
			...
			//기본 데이터들 입력 처리
			//출력 순서
			setMenuPrintSize(<c:out value='${parentSize}'/>);
			$("#menuOrder").val("<c:out value='${result.menuOrder}'/>").attr("selected", "selected");
			//언어코드			
			$("#codeLang").val("<c:out value='${result.codeLangType}'/>").attr("selected", "selected");
			//메뉴 코드
			$("#codeMenu").val("<c:out value='${result.codeMenuType}'/>").attr("selected", "selected");
			//로그인 여부
			$("#codeLogin").val("<c:out value='${result.codeLoginType}'/>").attr("selected", "selected");
			//부모 메뉴 셋팅
			$("#parentSeq").val(<c:out value='${result.parentSeq}' />);
			
			//최상위 메뉴확인
			if(<c:out value='${result.parentSeq}'/> == 0){
				//최상위 메뉴 선택 처리
				$("#thisTop").prop('checked', true);
			}else{
				$("#parentMenuName").html("<c:out value='${result.parentName}'/>");
			}
			if($("#codeMenu").val() == 'quick'){
			...
			$("#codeMenu").change(function(){
				...
				if(codeMenu == 'quick'){
					...
					var childSize = <c:out value="${childSize}" />;
					if(childSize > 0){
						alert('하위 메뉴가 있는 경우 퀵메뉴로 등록이 불가 합니다.');
						$("#codeMenu").val("<c:out value='${result.codeMenuType}'/>").attr("selected", "selected");
						return;
					}
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/menu/user/editMenu.edit' />", param)
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/menu/user/" + seq + ".view' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/menu/user/" + seq + ".view' />").submit();
