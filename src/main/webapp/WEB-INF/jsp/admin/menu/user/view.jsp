...
<html>
		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">사용자 메뉴 상세 조회</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="*">
						...
						<tbody>
							<tr>
								...
								<th>상위 메뉴 명</th>
								<td><c:out value="${result.parentName }"/></td>
							</tr>
							<tr>
								<th>출력 순서</th>
								<td><c:out value="${result.menuOrder }"/></td>
								<th>언어 코드</th>
								<td><c:out value="${result.codeLangName }"/></td>
							</tr>
							<tr>
								<th>메뉴 코드</th>
								<td><c:out value="${result.codeMenuName }"/></td>
								<th>로그인 여부</th>
								<td><c:out value="${result.codeLoginName }"/></td>
							</tr>
							<tr>
								<th>URL</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			let menuName = '<c:out value="${result.menuName }"/>';
			
			if(seq == ''){
				alert('존재하지 않는 사용자 메뉴 입니다.');
				location.href="<c:url value='/admin/menu/user/index.do' />";
			}
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm(menuName + ' 메뉴를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/menu/user/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
						    	history.back();
...
// 					location.href="<c:url value='/admin/menu/user/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/menu/user/"+ seq +".edit' />").submit();
