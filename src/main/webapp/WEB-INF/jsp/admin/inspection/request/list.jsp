...
<!-- 								</select> -->
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><c:out value="${item.thumFlag }"/></td>
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;검사의뢰서 추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
...
// 				location.href="<c:url value='/admin/inspection/request/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/inspection/request/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/inspection/request/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/inspection/request/add.write' />").submit();
