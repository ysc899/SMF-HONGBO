...
<html>
					...
					<form id="editForm">
						...
						<table>
							...
							<tbody>
								...
								<tr>
									<th>의뢰서 파일</th>
									<td colspan="3"><a href="<c:url value='/admin/inspection/request/download/${result.seq }.do'/>"><c:out value="${result.fileName}"/></a></td>
								</tr>
								</c:if>
								<tr>
									<th>의뢰서 파일 변경</th>
								...
								<c:if test="${result.thumFlag == 'Y' && not empty result.thumFilePath}">
								<tr>
									<th>섬네일</th>
	...
	<script type="text/javascript">
		...
		function getMaxPrintOrder(){
			...
			axios.get(url,{
			  ...
			  .then(function (res) {
				  ...
				  if(langCode != '<c:out value="${result.codeLangType}"/>' || thumFlag != '<c:out value="${result.thumFlag}"/>'){
					  size++;
				  }
		...
		$(document).ready(function(){
			...
			//섬네일 여뷰 처리
			if('<c:out value="${result.thumFlag}"/>' == 'Y' ){
				$("#thumFlag").prop("checked", "checked");
				$("#inputThum").show();
			}
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/inspection/request/editdRequest.edit' />", fd,{headers : {
						'Content-Type' : 'multipart/form-data'
					}})
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/inspection/request/${result.seq}.view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/inspection/request/${result.seq}.view' />").submit();
