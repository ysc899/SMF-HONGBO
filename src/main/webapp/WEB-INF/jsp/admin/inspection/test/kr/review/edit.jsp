		...
		<div class="notice_area">
			<form id="editForm">
				...
				<table>
					...
					<tr>
						<th class="th1">url</th>
						<td colspan="3"><input type="text" class="form-control text_title" name="t001url" id="t001url" value='<c:out value="${result.t001url }"/>'></td>
					</tr>
					</tbody>
				</table>
			</form>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#t001refSummernote").summernote("code", '<c:out escapeXml="false" value="${fn:escapeXml(result.t001ref)}"/>');
		...
		$("#t001contSummernote").summernote("code", '<c:out escapeXml="false" value="${fn:escapeXml(result.t001cont)}"/>');
		...
		$("#editForm").validate({
			...
			submitHandler: function(form) {
				...
				axios.post("<c:url value='/admin/inspection/test/kr/review/editInsp.edit' />", param)
						.then(function (res) {
							if(res.data.result){
								alert('수정요청이 등록 되었습니다.');
								location.href="<c:url value='/admin/inspection/test/kr/review/" + seq + ".view' />";
							}
