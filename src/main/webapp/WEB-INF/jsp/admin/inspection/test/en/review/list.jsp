					...
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘"><c:out value="${nowMenuName }"/> - 리스트</h1>
						<div class="select_box">
							...
							<form id="searchForm" method="get">
								...
								<select class="kind" name="dissClss" id="dissClss">
									<option value="" >전체</option>
...
<!-- 								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;동영상 추가</button> -->
	...
	<script type="text/javascript">
		...
		$("#dissClss").val('<c:out value="${param.dissClsOption }"/>').attr("selected", "selected");
		...
		function printPage(page){
			...
			for(startCursor; startCursor < endCursor; startCursor++){
				...
				contentHtml +='	<td><a href="javascript:void(0);">'+item.f010fen+'</a></td>';
		...
		function regInspectionData(datas){
			if(confirm(datas.length+'의 검사항목 검토 내용을 등록 하시겠습니까?')){
				...
				axios.post("<c:url value='/admin/inspection/test/en/review/excel.edit' />", {
...
// 				    }
		...
		$(document).ready(function(){
			...
			$("#btnDownloadExcel").click(function(){
				...
				axios({
					method: 'GET',
					url: "<c:url value='/admin/inspection/test/en/review/downexcel.do' />",
					params: param,
...
// 				$("#searchForm").attr('action', "<c:url value='/admin/inspection/test/en/review/downexcel.do' />").submit();
			});
...
// 								console.log(json[1]);
			...
			$("#btnAdd").click(function(){
				$("#searchForm").attr('action', "<c:url value='/admin/inspection/test/en/review/add.write' />").submit();
			});
