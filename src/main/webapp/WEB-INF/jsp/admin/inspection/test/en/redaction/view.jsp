				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>검사명</th>
								<td><c:out value="${result.asIs.f010fen }"/></td>
								<th>검사코드</th>
								<c:choose>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				...
				location.href="<c:url value='/admin/inspection/test/en/redaction/index.do' />";
			}
...
// 					t001day: {required:true, maxlength:100},
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/inspection/test/en/redaction/apply.edit' />", param)
					  .then(function (res) {
					    if(res.data.result){
					    	alert('적용 되었습니다.');
					    	location.href="<c:url value='/admin/inspection/test/en/redaction/" + seq + ".view' />";
					    }else{
			...
			$("._reject").click(function(){
				...
				if(confirm("반려 처리 하시겠습니까?")){
					...
					console.log(param);
					axios.post("<c:url value='/admin/inspection/test/en/redaction/reject.edit' />", param)
					  .then(function (res) {
					    if(res.data.result){
					    	alert('반려 되었습니다.');
					    	location.href="<c:url value='/admin/inspection/test/en/redaction/" + seq + ".view' />";
