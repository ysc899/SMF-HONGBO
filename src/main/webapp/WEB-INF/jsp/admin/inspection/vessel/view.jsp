				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>검사항목</th>
								<td colspan="3"><c:out value="${result.inspectionName }"/></td>
							</tr>
							<tr>
								<th>보관 조건</th>
								<td colspan="3"><c:out value="${result.codeVesselscName }"/></td>
							</tr>
							<tr>
								<th>참고사항</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.comment }"/></td>
							</tr>
							<tr>
								<th>이미지</th>
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('검체용기 정보를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/inspection/vessel/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/inspection/vessel/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/inspection/vessel/"+ seq +".edit' />").submit();
