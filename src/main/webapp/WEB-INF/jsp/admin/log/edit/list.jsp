						...
						<div class="select_box">
							...
							<form id="searchForm" method="get">
								...
								<select class="kind2" name="searchOption" id="optSearch">
									...
									<option value="userId">수정자 ID</option>
								</select>
							...
							<tbody>
								<c:forEach var="item" items="${result.content }" >
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<c:if test="${item.logType =='add' }">
											<td>추가</td>
										</c:if>
										<c:if test="${item.logType =='edit' }">
											<td>수정</td>
										</c:if>
										<td><a href="javascript:void(0);" style="color:#000;"><c:out value="${item.menuName }"/></a></td>
										<td><c:out value="${item.editUserId }"/></td>
										<td><c:out value="${item.editUserName }"/></td>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("._row").click(function(){
				...
				location.href="<c:url value='/admin/log/edit/" + seq + ".view' />";
			});
			...
			$("#btnExcel").click(function(){
				...
				axios({
					method: 'GET',
					url: "<c:url value='/admin/log/edit/excel.do' />",
					params: param,
				...
				.then(function(response){
					...
					var fileName = "수정_로그_"+param.startDate+"-"+param.endDate +'.xlsx';
			...
			$("#btnModalRemove").click(function(){
				...
				if(confirm(startDate+ '와 '+endDate+'일 사이의 수정 로그가 삭제됩니다.\n삭제시 복구 할 수 없습니다.\n수정 로그를 삭제 하시겠습니까?')){
					...
					axios.post("<c:url value='/admin/log/edit/removes.delete' />", param)
					  .then(function (res) {
				    	alert(res.data.result+'건의 수정 로그가 삭제 되었습니다.');
