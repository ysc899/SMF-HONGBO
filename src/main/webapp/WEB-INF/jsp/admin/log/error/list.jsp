							...
							<tbody>
								<c:forEach var="item" items="${result.content }" >
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><a href="javascript:void(0);" style="color:#000;"><c:out value="${item.url }"/></a></td>
										<td><c:out value="${item.regUserId }"/></td>
										<td><c:out value="${item.regUserName }"/></td>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("._row").click(function(){
				...
				location.href="<c:url value='/admin/log/error/" + seq + ".view' />";
			});
			...
			$("#btnExcel").click(function(){
				...
				axios({
					method: 'GET',
					url: "<c:url value='/admin/log/error/excel.do' />",
					params: param,
				...
				.then(function(response){
					...
					var fileName = "에러_로그_"+param.startDate+"-"+param.endDate +'.xlsx';
			...
			$("#btnModalRemove").click(function(){
				...
				if(confirm(startDate+ '와 '+endDate+'일 사이의 에러 로그가 삭제됩니다.\n삭제시 복구 할 수 없습니다.\n에러 로그를 삭제 하시겠습니까?')){
					...
					axios.post("<c:url value='/admin/log/error/removes.delete' />", param)
					  .then(function (res) {
				    	alert(res.data.result+'건의 에러 로그가 삭제 되었습니다.');
