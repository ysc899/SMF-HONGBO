...
<html>
					...
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘"><c:out value="${nowMenuName }"/> - 리스트</h1>
						<div class="select_box">
							...
							<form id="searchForm" method="get">
								...
								<select class="kind" name="codeCategoryType" id="optCategory">
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><c:out value="${item.codeCategoryName }"/></td>
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;학술제작물 추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
	...
	<script type="text/javascript">
		...
		$("#optCategory").val('<c:out value="${sCategory }"/>').attr("selected", "selected");
...
// 				location.href="<c:url value='/admin/promotion/ebook/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/promotion/ebook/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/promotion/ebook/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/promotion/ebook/add.write' />").submit();
