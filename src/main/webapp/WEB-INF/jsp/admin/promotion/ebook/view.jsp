...
<html>
				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>카테고리</th>
								<td><c:out value="${result.codeCategoryName }"/></td>
								<th>출력순서</th>
							...
							<tr>
								<th>URL</th>
								<td colspan="3"><a href='<c:out value="${result.url }"/>' target="_blank"><c:out value="${result.url }"/></a></td>
							</tr>
							<tr>
								<th>섬네일</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 학술제작물 입니다.');
				location.href="<c:url value='/admin/promotion/ebook/index.do' />";
			}
			
			//목록 가기 클릭 이벤트
			$("._list").click(function(){
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/ebook/index.do' />").submit();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('학술제작물을 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/promotion/ebook/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/promotion/ebook/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/ebook/"+ seq +".edit' />").submit();
