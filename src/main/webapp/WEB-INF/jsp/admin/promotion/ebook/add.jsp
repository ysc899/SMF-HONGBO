						...
						<table>
							...
							<tbody>
								...
								<tr>
									<th>URL</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="url" placeholder="등록될 URL을 입력해주세요." id="url">
									</td>
								</tr>
								<tr>
									<th>섬네일 이미지</th>
		...
		$(document).ready(function(){
			...
			$("#addForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/promotion/ebook/addeBook.write' />", fd,{headers : {
						'Content-Type' : 'multipart/form-data'
					}})
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('등록 되었습니다.');
//  					    	location.href="<c:url value='/admin/promotion/ebook/" + res.data.result + ".view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/ebook/" + res.data.result + ".view' />").submit();
