		...
		$(document).ready(function(){
			...
			$("#addForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/promotion/image/addPromotion.write' />", fd,{headers : {
						'Content-Type' : 'multipart/form-data'
					}})
					  .then(function (res) {
					    if(res.data.result.stateCode > 0){
					    	alert('등록 되었습니다.');
//  					    	location.href="<c:url value='/admin/promotion/image/" + res.data.result.seq + ".view' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/image/" + res.data.result.seq + ".view' />").submit();
...
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/image/index.do' />").submit();
