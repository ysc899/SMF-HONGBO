		...
		$(document).ready(function(){
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/promotion/insp/editPromotion.edit' />", fd,{headers : {
						'Content-Type' : 'multipart/form-data'
					}})
					  .then(function (res) {
					    if(res.data.result == 1){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/promotion/insp/${result.seq}.view' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/insp/${result.seq}.view' />").submit();

					    }else if(res.data.result == -7){
					    	alert('섬네일 자동 생성에 실패 하였습니다.');
// 					    	location.href="<c:url value='/admin/promotion/insp/${result.seq}.edit' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/insp/${result.seq}.view' />").submit();
