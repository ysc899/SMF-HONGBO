								...
								<tr >
									...
									<td colspan="3">
										<video id="player" playsinline controls style="width:720px; height:405px;">
										  <source src="<c:url value='/admin/promotion/video/stream/${result.seq }.view'/>" type="video/mp4" />
										</video>
									</td>
								</tr>
								<tr>
		...
		$(document).ready(function(){
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/promotion/video/editPromotion.edit' />", fd,{headers : {
						'Content-Type' : 'multipart/form-data'
					}})
					  .then(function (res) {
					    if(res.data.result == 1){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/promotion/video/${result.seq}.view' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/video/${result.seq}.view' />").submit();

					    }else if(res.data.result == -7){
					    	alert('섬네일 자동 생성에 실패 하였습니다.');
// 					    	location.href="<c:url value='/admin/promotion/video/${result.seq}.edit' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/video/${result.seq}.view' />").submit();
