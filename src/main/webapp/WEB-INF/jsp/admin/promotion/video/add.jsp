...
<html>
	...
	<script type="text/javascript">
		...
		function getVideoSize(){
			...
			let url = "<c:url value='/admin/promotion/video/maxPrintOrder.json' />";
				...
				submitHandler: function(form) {
					...
					if(param.url == ''){
						if(promotionFile.length == 0){
							alert('재생 URL 이나 파일 둘중 하나는 필수 입니다.');
							return;
						}else{
						...
						if(promotionFile.length > 0){
							alert('재생 URL 이나 파일을 동시에 등록은 불가능 합니다.');
							return;
						}else{
							if(thumFile.length == 0){
					...
					axios.post("<c:url value='/admin/promotion/video/addPromotion.write' />", fd,{headers : {
						'Content-Type' : 'multipart/form-data'
					}})
					  .then(function (res) {
					    if(res.data.result.stateCode > 0){
					    	alert('등록 되었습니다.');
//  					    	location.href="<c:url value='/admin/promotion/video/" + res.data.result.seq + ".view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/video/" + res.data.result.seq + ".view' />").submit();
...
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/promotion/video/index.do' />").submit();
