						...
						<div class="select_box">
							...
							<form id="searchForm" method="get">
								...
								<select class="kind" name="searchOption" id="optSearch">
									...
									<option value="name">코드 이름</option>
								</select>
						...
						<table>
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-id="<c:out value="${item.seq }"/>">
										<td><c:out value="${item.row }"/></td>
										<td><a href="javascript:void(0);" style="color:#000;"><c:out value="${item.masterCode }"/></a></td>
										<td><c:out value="${item.masterCodeName }"/></td>
										<td><c:out value="${item.systemCodeFlag }"/></td>
...
<!-- 							<button id="btnAddRole" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-user-plus"></i>&nbsp;코드 추가</button> -->
						</div>
				</div>
				<!--/notice_area End-->
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("._row").click(function(){
				var id = $(this).data('id');
// 				location.href="<c:url value='/admin/code/" + id + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/code/" + id + ".view' />").submit();
...
// // 					location.href="<c:url value='/admin/code/addRole.write' />";
// 					$("#searchForm").attr('action', "<c:url value='/admin/code/addRole.write' />").submit();
