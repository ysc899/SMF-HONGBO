						...
						<table>
							...
							<tbody>
								...
								<tr>
									<th>내용</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="content" placeholder="등록될 롤링 내용을 입력해주세요." id="content" value="<c:out value='${result.content }'/>">
									</td>
								</tr>
								<tr>
									<th>링크 URL</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="url" placeholder="등록될 링크 URL을 입력해주세요." id="url" value="<c:out value='${result.url }'/>">
									</td>
								</tr>
							</tbody>
						</table>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("#codeLang").change(function(){
				...
				let url = "<c:url value='/admin/main/rolling/getPrintOrder.json' />";
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/main/rolling/editRolling.edit' />", param)
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/main/rolling/${result.seq}.view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/main/rolling/${result.seq}.view' />").submit();
