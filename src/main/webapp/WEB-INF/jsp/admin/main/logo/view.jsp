...
<html>
				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>로고 종류</th>
								<td colspan="3"><c:out value="${result.codeLogoName }"/></td>
							</tr>
							<tr>
								<th>이름</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.name }"/></td>
							</tr>
							<c:if test="${result.codeLogoType == 'left'}">
							<tr>
								<th>url</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.url }"/></td>
							</tr>
							
							</c:if>
							<tr>
								<th>로고</th>
								<td colspan="3"><img src='<c:out escapeXml="false" value="${result.logoImage }"/>' style="width:auto;"></td>
							</tr>
							<tr>
								<th>시작일</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 로고정보 입니다.');
				location.href="<c:url value='/admin/main/logo/index.do' />";
			}
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('로고 정보를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/main/logo/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
				...
				$("._edit").click(function(){
					$("#hiddenForm").attr('action', "<c:url value='/admin/main/logo/"+ seq +".edit' />").submit();
