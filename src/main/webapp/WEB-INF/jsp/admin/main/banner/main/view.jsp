...
<html>
					...
					<table>
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="3">
									<c:choose>
										<c:when test="${result.url ne '' }" >
											<a href="<c:out value='${result.url }'/>" target="_blank"> <c:out value="${result.url }"/></a>
										</c:when>
										<c:otherwise>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 서브 베너 입니다.');
				location.href="<c:url value='/admin/main/banner/main/index.do' />";
			}
			
			//목록 가기 클릭 이벤트
			$("._list").click(function(){
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/main/banner/main/index.do' />").submit();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('서브 베너 정보를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/main/banner/main/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/main/banner/main/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/main/banner/main/"+ seq +".edit' />").submit();
