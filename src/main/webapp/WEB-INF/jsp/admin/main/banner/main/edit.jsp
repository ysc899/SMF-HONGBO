...
<html>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<input type="checkbox" id="prinfFlag">
									</td>
								</tr>
								<tr>
									<th>타이틀</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="title" placeholder="수정될 베너 타이틀을 입력해주세요." id="title" value="<c:out value='${result.title }'/>">
									</td>
								</tr>
								<tr>
									<th>내용</th>
								...
								<tr>
									<th>링크 URL</th>
									<td colspan="3">
										<input type="text" class="form-control text_title" name="url" placeholder="수정될 링크 URL을 입력해주세요." id="url" value="<c:out value='${result.url }'/>">
									</td>
								</tr>
								<tr>
									<th>이미지</th>
									<td colspan="3"><img src='<c:out escapeXml="false" value="${result.fileContent }"/>' style="width:100%;"></td>
								</tr>
								<tr>
									<th>수정 이미지</th>
									<td colspan="3">
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			//순서 관련 처리
			appendPrintOrder(<c:out value="${defaultInfo.maxCount}"/>);
			$("#printOrder").val(<c:out value="${result.printOrder}"/>).attr("selected", "selected");
			
			//출력 여부 관련 처리
			if('<c:out value="${result.printFlag}"/>' == 'Y'){
				$("#prinfFlag").prop('checked', true);
			}
			
			//언어코드 초기화
			$("#codeLang").val('<c:out value="${result.codeLangType}"/>').attr("selected", "selected");
			
			let content = '<c:out value="${result.content }"/>';
			$("#content2").val(CommonUtil.toEnter(CommonUtil.unescapeHtml(content)));
			...
			$("#codeLang").change(function(){
				...
				let url = "<c:url value='/admin/main/banner/main/getPrintOrder.json' />";
				...
				axios.get(url,{
				  ...
				  .then(function (res) {
					  ...
					  if(codeVal != '<c:out value="${result.codeLangType}"/>'){
						  size++;
					  }
					  
					  appendPrintOrder(size);
				  })
			...
			$("#editForm").validate({
				rules:{
					title: {required:true, maxlength:100, minlength:2},
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/main/banner/main/editBanner.edit' />", fd,{headers : {
						'Content-Type' : 'multipart/form-data'
					}})
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/main/banner/main/${result.seq}.view' />";
							$("#hiddenForm").attr('action', "<c:url value='/admin/main/banner/main/${result.seq}.view' />").submit();
