...
<html>
				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th>로그인 여부</th>
								<td colspan="3"><c:out value="${result.codeLoginName }"/></td>
							</tr>
							<tr>
								<th>타이틀</th>
							...
							<tr>
								<th>출력 여부</th>
								<td colspan="3"><c:out value="${result.printFlag }"/></td>
							</tr>
							<tr>
								<th>시작일</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('존재하지 않는 팝업정보 입니다.');
				location.href="<c:url value='/admin/main/popup/index.do' />";
			}
...
// 				history.back();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('팝업 정보를 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/main/popup/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/main/popup/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/main/popup/"+ seq +".edit' />").submit();
