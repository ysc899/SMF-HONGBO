		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">통합 FAQ 관리</h1>
						<div class="select_box">
							...
							<form id="searchForm" method="get">
								...
								<select class="kind" name="searchOption" id="optSearch">
									...
									<option value="name">FAQ 명</option>
							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><a href="javascript:void(0);" style="color:#000;"><c:out value="${item.masterName }"/></a></td>
										<td><c:out value="${item.contentCount }"/></td>
						...
						<div class="page_box">
							...
							<c:if test="${writeFlag == 'true' || writeFlag == true }">
								<button id="btnAddNotice" type="button" class="btn btn-default" style="float:right; display: none;" ><i class="fas fa-plus"></i>&nbsp;FAQ 추가</button>
							</c:if>
						</div>
				</div>
				<!--/notice_area End-->
...
// 				location.href="<c:url value='/admin/faq/manage/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/faq/manage/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/faq/manage/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/faq/manage/add.write' />").submit();
