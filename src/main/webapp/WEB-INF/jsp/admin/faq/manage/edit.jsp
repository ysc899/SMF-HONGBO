...
<html>
			...
			<div id="container">
				...
				<div class="notice_area">
					<form id="editForm">
						...
						<sec:csrfInput />
						<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">통합 FAQ 수정</h1>
						<table>
							<colgroup>
								<col width="100px;"/>
								<col width="*;"/>
							...
							<tbody>
								...
								<tr>
									<th>FAQ 명</th>
									<td>
										<input type="text" class="form-control text_title" name="name" placeholder="등록될 FAQ명을 입력해주세요." id="name" value='<c:out value="${result.masterName }"/>'>
									</td>
								</tr>
								<tr>
									<th>FAQ 설명</th>
									<td><textarea class="form-control" rows="3" cols="100" id="desc" name="desc" style="resize:none;"></textarea></td>
								</tr>
							</tbody>
						</table>
					</form>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			let desc = '<c:out escapeXml="false" value="${result.masterDesc }"/>';
			...
			$("#editForm").validate({
				...
				submitHandler: function(form) {
					...
					axios.post("<c:url value='/admin/faq/manage/editFaqManage.edit' />", param)
					  .then(function (res) {
					    if(res.data.result > 0){
					    	alert('수정 되었습니다.');
//  					    	location.href="<c:url value='/admin/faq/manage/" + seq + ".view' />";
 							$("#hiddenForm").attr('action', "<c:url value='/admin/faq/manage/" + seq + ".view' />").submit();
