...
<html>
		...
		<div id="wrap">
			...
			<div id="container">
				...
				<div class="notice_area">
					<h1><img src="<c:url value='/admin/img/menu.svg'/>" alt="메뉴아이콘">통합 FAQ 상세 조회</h1>
					<table>
						<colgroup>
							<col width="150px;">
							<col width="*">
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			let faqName = '<c:out value="${result.masterName }"/>';
			if(seq == ''){
				alert('존재하지 않는 FAQ 입니다.');
				location.href="<c:url value='/admin/faq/manage/index.do' />";
			}
			
			//목록 가기 클릭 이벤트
			$("._list").click(function(){
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/faq/manage/index.do' />").submit();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm(faqName + ' FAQ을 삭제 하시겠습니까?')){
						
						axios.post("<c:url value='/admin/faq/manage/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
...
// 					location.href="<c:url value='/admin/faq/manage/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/faq/manage/"+ seq +".edit' />").submit();
