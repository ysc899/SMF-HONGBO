							...
							<tbody>
								<c:forEach var="item" items="${result.content }">
									<tr class="_row" data-seq="<c:out value="${item.seq }"/>">
										...
										<td><a href="javascript:void(0);" style="color:#000;"><c:out value="${item.question }"/></a></td>
										<td><c:out escapeXml="false" value="${item.answer }"/></td>
...
// 				location.href="<c:url value='/admin/faq/integrated/${masterSeq}/" + seq + ".view' />";
				$("#searchForm").attr('action', "<c:url value='/admin/faq/integrated/${masterSeq}/" + seq + ".view' />").submit();
...
// 					location.href="<c:url value='/admin/faq/integrated/${masterSeq}/add.write' />";
					$("#searchForm").attr('action', "<c:url value='/admin/faq/integrated/${masterSeq}/add.write' />").submit();
				});
			}
