				...
				<div class="notice_area">
					...
					<table>
						...
						<tbody>
							<tr>
								<th>카테고리</th>
								<td colspan="3"><c:out value="${result.codeCategoryName }"/></td>
							</tr>
							<tr>
								<th>질문</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.question }"/></td>
							</tr>
							<tr>
								<th>답변</th>
								<td colspan="3"><c:out escapeXml="false" value="${result.answer }"/></td>
							</tr>
							<tr>
								<th>최종 수정자</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				...
				location.href="<c:url value='/admin/faq/integrated/${masterSeq}/index.do' />";
			}
			
			//목록 가기 클릭 이벤트
			$("._list").click(function(){
// 				history.back();
				$("#hiddenForm").attr('action', "<c:url value='/admin/faq/integrated/${masterSeq}/index.do' />").submit();
			...
			if(writeFlag){
				...
				$("._remove").click(function(){
					if(confirm('내용을 삭제 하시겠습니까?')){
						...
						axios.post("<c:url value='/admin/faq/integrated/${masterSeq}/"+ seq +".delete' />")
						  .then(function (res) {
						    if(res.data.result){
						    	alert('삭제 되었습니다.');
// 						    	history.back();
...
// 					location.href="<c:url value='/admin/faq/integrated/${masterSeq}/"+ seq +".edit' />";
					$("#hiddenForm").attr('action', "<c:url value='/admin/faq/integrated/${masterSeq}/"+ seq +".edit' />").submit();
