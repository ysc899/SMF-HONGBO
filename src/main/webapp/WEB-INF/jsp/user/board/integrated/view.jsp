...
<!-- 	<meta name="X-UA-Compatible" content="IE=edge,chrome=1"/> -->
    ...
    <meta property="og:image" content="<c:url value='/board/${masterSeq}/thum/${result.seq }.view'/>" />
	...
	<link href="<c:url value='/user/css/05_sub01_inner.css'/>" type="text/css" rel="stylesheet"/>
...
<body>
					...
					<div class="sub_story">
						<div class="notice_inner">
							<table class="table1">
								...
								<tbody>
									<tr>
										<th><spring:message code="board.view.title" /></th>
										<td><c:out escapeXml="false" value="${result.title }"/></td>
										<th><spring:message code="board.view.writer" /></th>
										<td><spring:message code="title" /></td>
									</tr>

									<tr>
										<th><spring:message code="board.view.date" /></th>
										<td><fmt:formatDate pattern = "yyyy-MM-dd" value = "${result.editDate}" /></td>
										<th><spring:message code="board.view.count" /></th>
										<td><c:out value="${result.hitCnt}"/></td>
									</tr>
									
									<c:if test="${option.optFileFlag == 'Y' &&  fn:length(result.files) > 0}">
									<tr>
										<th><spring:message code="board.view.file" /></th>
										<td colspan="3" class="td_zip">
											<ul>
												<c:forEach items="${result.files }" var="item">
													<li class="_fileDown" data-seq="<c:out value='${item.seq }'/>"><img src="<c:url value='/user/img/sub/folder.png' />" class="folder_ico" alt="폴더아이콘"><c:out value="${item.fileName }"/></img></li>
												</c:forEach>
							...
							<table class="table2">
								...
								<tbody>
									<tr>
										<c:choose>
											<c:when test="${result.preBoard.seq > 0 }">
												<th style="border-bottom:1px solid #ccc;" class="th_sil"><spring:message code="search.pre" /></th>
												<td class="_move" data-seq="<c:out value='${result.preBoard.seq }'/>" style="border-bottom:1px solid #ccc; cursor: pointer;"><c:out value="${result.preBoard.title }"/></td>
											</c:when>
											<c:otherwise>
									...
									<tr>
										<c:choose>
											<c:when test="${result.nextBoard.seq > 0 }">
												<th style="border-bottom:1px solid #ccc;" class="th_sil"><spring:message code="search.next" /></th>
												<td class="_move" data-seq="<c:out value='${result.nextBoard.seq }'/>" style="border-bottom:1px solid #ccc; cursor: pointer;"><c:out value="${result.nextBoard.title }"/></td>
											</c:when>
											<c:otherwise>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				alert('<spring:message code="board.view.no.seq" />');
				location.href="<c:url value='/board/${masterSeq}/index.do' />";
			}
			

			$("._golist").click(function(){
				$("#hiddenForm").attr('action', "<c:url value='/board/${masterSeq}/index.do' />").submit();
			});
			$("._fileDown").click(function(){
				var fileSeq = $(this).data('seq');
				location.href="<c:url value='/board/${masterSeq}/" + seq + "/download/" + fileSeq + ".do' />";
			});
			
			$("._move").click(function(){
				var seq = $(this).data('seq');
				$("#hiddenForm").attr('action', "<c:url value='/board/${masterSeq}/" + seq + ".view' />").submit();
			});
