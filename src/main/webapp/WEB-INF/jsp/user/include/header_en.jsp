...
<%-- 					<img src="<c:url value='/user/img/logo_left_en_1.png'/>" class="left_logo"	alt="좌측로고" />
			...
			<form method="post" action="" name="register" id="formLogin">
				...
				<table>
					<tbody>
						<tr>
							<td>
								<input type="text" name="userId" maxlength="18" placeholder="ID" class="table_id" id="headerId"/>
							</td>
						</tr>

						<tr>
							<td>
								<input type="password" name="pwd" maxlength="20" placeholder="Password" id="pwd">
							</td>
						</tr>

						<tr>
							<td>
								<button type="button" name="로그인" value="Login" id="btnLogin">Login</button>
							</td>
						</tr>
					</tbody>
				</table>
	...
	<c:choose>
		...
		<input type="button" value="TO VIEW TEST RESULT" id="goTrms" class="_goTrms">
		</c:when>
		<c:otherwise>
			<br/><br/><br/>
		</c:otherwise>
			...
			<a href="javascript:void(0);" class="_logout" >Logout</a>
		</div>
...
<%-- 		<b class="close_bt _pwModalClose"><img src="<c:url value='/user/img/sub/close.png'/>" alt="닫기버튼"/></b>  --%>
	...
	<div class="popup_inner">
		...
		<div class="ok_area">
			<span id="btnChangePw">Submit</span>
		</div>
...
// 						window.location.reload();
					...
					}else if(res.data.resultCode.code == 4){
						alert('사용 중지된 병원입니다.');
