		...
		<c:choose>
			<c:when test="${not empty logos.leftLogo }">
				<%-- <img src="<c:out escapeXml="false" value='${logos.leftLogo.logoImage }'/>" class="left_logo _leftLogoFixClick" data-url="<c:out value='${logos.leftLogo.url }'/>"	alt="좌측로고" /> --%>
					<img src="<c:url value='/user/img/logo_left_en_1.png'/>" class="left_logo"	alt="좌측로고" />
					<img src="<c:url value='/user/img/logo_left_en_2.png'/>" class="left_logo"	alt="좌측로고" />
			</c:when>
			<c:otherwise>
				<%-- <img src="<c:url value='/user/img/logo_left_en.png'/>" class="left_logo"	alt="좌측로고" /> --%>
				<%-- <img src="<c:url value='/user/img/phone_en.png'/>" class="left_logo"	alt="좌측로고" /> --%>
				<%-- <img src="<c:url value='/user/img/logo_left_en_1.png'/>" class="left_logo"	alt="좌측로고" />
				<img src="<c:url value='/user/img/logo_left_en_2.png'/>" class="left_logo"	alt="좌측로고" /> --%>
				<img src="<c:url value='/user/img/logo_new_2024_en.png'/>" class="left_logo"	alt="좌측로고" />	 
			</c:otherwise>
		</c:choose>
		<c:choose>
			...
			<ul>
		 	<c:forEach items="${menus.topMenus}" var="parentMenu" varStatus="status">
				...
				<li onclick="location.href='#'" id="not_gnb" class="not_gnb">
					<a href="#" class="test_text">Test result</a>
				</li>
									...
									<li data-flag="n" class="li_bt">
										...
										<ul class="off">
											<c:forEach items="${childMenu.childs}" var="l2Child">
												<c:choose>
													<c:when test="${empty l2Child.menuUrl}">
														<li style="font-size: 14px; color: #666;">- <c:out value="${l2Child.menuName }"/></li>
													</c:when>
													<c:otherwise>
					...
					<ul class="g_sub_menu gnb_off07">
						<li class="fix_login">
								...
								<form method="post" action="" name="register" id="formLogin2">
									...
									<table>
										<tbody>
											<tr>
												<td>
													<input type="text" name="userId" maxlength="18" placeholder="ID" value="" class="table_id" id="fixHeaderId">
												</td>
											</tr>

											<tr>
												<td>
													<input type="password" name="pwd" maxlength="20" placeholder="Password" id="pwd2" >
												</td>
											</tr>

											<tr>
												<td>
													<button type="button" name="로그인" value="Login" id="btnLogin2">Login</button>
												</td>
											</tr>
										</tbody>
									</table>
										...
										<label for="idsave02">Save ID</label>
							...
							<div class="login_box_on02">
								...
								<p>
									Welcom <br/><span class="user_name"><c:out value="${userName }"/></span>
								</p>
								
								<c:choose>
									<c:when test="${not empty trmsUrl}">
									<input type="button" value="TO VIEW TEST RESULT" class="_goTrms2">
									</c:when>
									<c:otherwise>
										<br/><br/><br/>
									</c:otherwise>
								...
								<a href="javascript:void(0);" class="_logout2" >Logout</a>
							</div>
							</sec:authorize>
						</li>
					</ul>
...
<div id="popup">
	<div class="popup_title">
		...
		<span id="paper_name">Change Password</span>
