...
<!-- 	<meta name="X-UA-Compatible" content="IE=edge,chrome=1"/> -->
    ...
    <meta property="og:image" content="<c:url value='/customer/payrollNotice/thum/${result.seq }.view'/>" />
	...
	<div id="wrap">
		...
		<!--container-->
			<div id="container">
					...
					<div id="sub_title">
						<p><c:out value="${result.payrollNoticeNumber}"/></p>
					...
					<div class="sub_story">
						<div class="notice_inner">
							<table class="table1">
								...
								<tbody>
									<tr>
										<th><spring:message code="payrollNotice.list.number" /></th>
										<td colspan="3"><c:out value="${result.payrollNoticeNumber}"/></td>
									</tr>
									<tr>
										<th><spring:message code="payrollNotice.view.title" /></th>
										<td><c:out value="${result.title }"/></td>
										<th><spring:message code="payrollNotice.view.writer" /></th>
										<td><spring:message code="title" /></td>
									</tr>

									<tr>
										<th><spring:message code="payrollNotice.view.date" /></th>
										<td><fmt:formatDate pattern = "yyyy-MM-dd" value = "${result.editDate}" /></td>
										<th><spring:message code="payrollNotice.view.count" /></th>
										<td><c:out value="${result.hitCnt}"/></td>
									</tr>

									<tr>
										<th><spring:message code="payrollNotice.view.file" /></th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(seq == ''){
				...
				location.href="<c:url value='/customer/payrollNotice/index.do' />";
			}
			

			$("._golist").click(function(){
				$("#hiddenForm").attr('action', "<c:url value='/customer/payrollNotice/index.do' />").submit();
			});

			$("._fileDown").click(function(){
				var fileSeq = $(this).data('seq');
				location.href="<c:url value='/customer/payrollNotice/download/" + seq + "/" + fileSeq + ".do' />";
			});
			
			
			$("._move").click(function(){
				var seq = $(this).data('seq');
				$("#hiddenForm").attr('action', "<c:url value='/customer/payrollNotice/" + seq + ".view' />").submit();
			});
