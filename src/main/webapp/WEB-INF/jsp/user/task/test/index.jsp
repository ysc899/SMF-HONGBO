...
<%-- 								<img src="<c:url value='/user/img/search.png'/>" alt="search" class="_search"/> --%>
								...
								<span class="search _search">검색</span>
							</div>
													...
													<c:forEach items="${searchOption.dissClss}" var="item" varStatus="status">
														<c:if test="${status.count % 6 == 1}">
															...
															<td>
																...
																<label for="diss_<c:out value='${status.count }'/>"><c:out value='${item.s002cnm }'/></label>
															</td>
...
// 		//자세히보기 클릭 이벤트
		...
		function printPage(page){
			...
			for(startCursor; startCursor < endCursor; startCursor++){
				...
				contentHtml +='	<td style="max-width: 0px;overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-align: left; padding: 0 5px 0 5px;">'+item.f010fkn+'</td>';
