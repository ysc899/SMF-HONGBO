...
<!-- 	<meta name="X-UA-Compatible" content="IE=edge,chrome=1"/> -->
	...
	<link href="<c:url value='/user/css/02_sub03_en.css?ver=20230830'/>" type="text/css" rel="stylesheet"/>
		...
		<!--container-->
			<div id="container">
					...
					<div class="sub_story">
						...
						<ul class="big_tabs tabs">


								<li class="tab-link center01 _certRow _center _center_seoul" data-center="seoul" data-tab="tab-1"><span>Central Laboratory</span></li>

								<li class="tab-link center02 _certRow _center _center_busan" data-center="busan" data-tab="tab-2"><span>Busan & Gyeongnam <br/>Laboratory Center</span></li>

								<li class="tab-link center03 _certRow _center _center_daegu" data-center="daegu" data-tab="tab-3"><span>Daegu & Gyeongbuk <br/>Laboratory Center</span></li>

								<li class="tab-link center04 _certRow _center _center_gwangju" data-center="gwangju" data-tab="tab-4"><span>Gwangju & Honam <br/>Laboratory Center</span></li>

								<li class="tab-link center05 _certRow _center _center_daejeon" data-center="daejeon" data-tab="tab-5"><span>Daejeon & Chungcheong <br/>Laboratory Center</span></li>


<%--							<c:forEach items="${defaultInfo.centerCodes }" var="item" varStatus="status">--%>
<%--								<li class="tab-link center0<c:out value='${status.count }'/> _certRow _center _center_<c:out value='${item.slaveCode }'/>" data-center="<c:out value='${item.slaveCode }'/>"--%>
<%--									data-tab="tab-<c:out value='${status.count }'/>"--%>
<%--								><c:out value='${item.codeName }'/></li>--%>
<%--							</c:forEach>--%>
...
<!-- 						 <li class="center02 _center _center_other" data-center="other">지역검사센터<span></span></li> -->
						...
						<c:forEach items="${defaultInfo.centerCodes }" var="ctitem" varStatus="ctstatus">
							...
							<div id="tab-<c:out value='${ctstatus.count }'/>" class="tab-content tab_common">
								<div class="one_search">
									<form id="<c:out value='${ctitem.slaveCode }'/>Form" onSubmit="return false;">
										<span><spring:message code="search.msg" /></span>
										<input type="text" class="_searchKeyword" name="searchKeyword" placeholder="<spring:message code="search.hint" />" data-center="<c:out value='${ctitem.slaveCode }'/>"/>
										<img class="_search" src="<c:url value='/user/img/search.png'/>" alt="검색" data-center="<c:out value='${ctitem.slaveCode }'/>"/>
										<input type="hidden" name="currentPage" value="0" id="<c:out value='${ctitem.slaveCode }'/>Current"/>
										<input type="hidden" name="codeCenterType" value="<c:out value='${ctitem.slaveCode }'/>"/>
									</form>
								</div>

								<div class="sil_box" id="<c:out value='${ctitem.slaveCode }'/>List">
								</div>

								<div class="one_click_more_view _more" id="<c:out value='${ctitem.slaveCode }'/>More" data-center="<c:out value='${ctitem.slaveCode }'/>">
...
<%--			<jsp:include  page="../../include/rolling.jsp"></jsp:include >--%>
	...
	<script type="text/javascript">
		...
		function search(center){
			...
			$('#'+center+'List').html('');
			$('#'+center+'Current').val(0);
			getData(center);
		}
		
		function getData(center){
			var param = $('#'+center +'Form').serializeObject();
			...
			axios.get(url,{
			  ...
			  .then(function (res) {
				  ...
				  <c:forEach items="${defaultInfo.certCodes }" var="citem" varStatus="cstatus">
					  <c:forEach items="${defaultInfo.certLocCodes }" var="item" varStatus="status">

					  var <c:out value='${item.slaveCode }'/> = _.filter(res.data.result.content, function(t){return t.codeCertLocType == "<c:out value='${item.slaveCode }'/>" && t.codeCertType == "<c:out value='${citem.slaveCode }'/>"

					  });
					  if(<c:out value='${item.slaveCode }'/>.length > 0){
						  content +=  '<p class="sub_title">${item.codeName } - ${citem.codeName }</p>';
				  ...
				  $('#'+center+'List').append(content);
				  $('#'+center+'Current').val(res.data.result.currentPage);
...
<%--			<c:forEach items="${defaultInfo.certCodes }" var="item" varStatus="status">--%>
				getData("<c:out value='${ctitem.slaveCode }'/>");
<%--			</c:forEach>--%>
			</c:forEach>
