package com.seegene.web.user.board.news.controller;
...
import com.seegene.web.user.board.news.service.NewsUserService;
...
public class NewsUserController {
	...
	@Autowired
	private NewsUserService service;
	/**
	 * @param param 검색조건
	 * @param model
	 * @return
	...
	public String list(@PathVariable("masterSeq") final int masterSeq, final SearchIntegrateSearchPaggingPO param, final Locale locale, final Model model, final HttpServletRequest request) throws Exception{
		...
		if(locale.getLanguage().equals("ko")){
			return "/user/board/news/"+viewPage;
		}else{
			return "/user/board/news/en/"+viewPage;
		}
...
//		return "/user/board/news/view";
		if(locale.getLanguage().equals("ko")){
			return "/user/board/news/view";
		}else{
			return "/user/board/news/en/view";
		}
